...
module RubyMarks
  ...
  class Recognizer
    ...
    def detect_mark_blocks
      ...
      groups.each_pair do |label, group|
         marks_blocks = if config.scan_mode == :grid
            ...
            find_marks(original_file_str, group)
          end
    ...
    def find_marks(image, group)
      ...
      find_map_lines(blocks, group)

      delete_conflicts(blocks, group)

      bubbles_adjusted = verify_bubbles_adjust(blocks, group)

      incorrect_expected_lines = group.incorrect_expected_lines

      incorrect_bubble_line_found = verify_incorrect_bubble_line(group)
    ...
    def find_map_lines(marks_blocks, group)
      line = 0
      mark_ant = nil
    ...
    def delete_conflicts(marks_blocks, group)
      mark_ant = nil
      marks_blocks.each do |mark|
    ...
    def verify_bubbles_adjust(marks_blocks, group)
      bubbles_adjusted = []
      first_position = 0
    ...
    def verify_incorrect_bubble_line(group)
